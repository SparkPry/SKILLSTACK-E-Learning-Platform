...
async function fetchCourses() {
  try {
    ...
    const cardDisplay = data.content.map((pro) => {
      return `
       <div 
  class="bg-white rounded-2xl shadow-lg w-full p-5 dark:bg-gray-800 dark:text-gray-200 transition-all duration-300 hover:scale-105 group cursor-pointer"
  onclick="location.href='./pages/courseDetail.html?slug=${pro.slug}'"
>
  ...
  <div class="flex items-center justify-between">
    <div class="flex items-center space-x-2">
      <img src="./imgs/ISTAD.png" alt="ISTAD Logo" class="w-8 h-8 rounded-full" />
      <span class="text-gray-700 font-medium dark:text-white">ISTAD</span>
    </div>

    <div class="flex gap-2 items-end">
      <span class="text-gray-700 text-2xl font-bold dark:text-white">$${pro.discount}</span>
      <span class="text-gray-500 text-sm line-through dark:text-gray-300">$${pro.price}</span>
    </div>
  </div>
</div>
